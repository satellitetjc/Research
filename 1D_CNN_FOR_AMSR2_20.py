#说明1：利用一维cnn做土壤水分反演，输入数据是14个通道亮温，目标数据是amsr2的jaxa土壤水分产品，日数据，数据需转化成csv格式，存在相对路径里，在keras模块下运行。
#说明2：通道维卷积的新版本，加了batchnormalzation层，加了dropout层以防过拟合，2016年7月28日的数据，加了全连接层1000个节点，打乱了数据，验证样本和测试样本不是同一个
